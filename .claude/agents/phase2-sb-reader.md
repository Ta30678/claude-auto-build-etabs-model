...
每個 SB-READER 自行執行 `pptx_to_elements.py --phase phase2` 處理分配的樓層，
再進行 affine 校正 + sb_validate + AI 視覺驗證。
...
- `PAGE_FLOOR_MAPPING` — 你負責的 page-floors 對應（e.g. `"3=1F~2F, 4=3F~14F"`）
- `GRID_DATA` — grid_data.json 路徑
- `SLIDES_INFO_DIR` — Phase 1 的 `SLIDES INFO/` 路徑（供讀 grid_anchors + screenshots）
- `SB_SLIDES_INFO_DIR` — Phase 2 的 `SB SLIDES INFO/` 路徑（SB per-slide JSONs + calibrated 輸出位置）
...
先執行 `pptx_to_elements.py` 提取你負責的樓層的 SB JSON：
...
python -m golden_scripts.tools.pptx_to_elements \
    ...
    --phase phase2 \
    --slides-info-dir "{SB_SLIDES_INFO_DIR}"
```

**輸出**：每個 floor 在 `SB SLIDES INFO/{fl}/pptx_to_elements/sb_{fl}.json` 生成 per-slide SB JSON（PPT-meter 座標）。

**驗證**：檢查每個 floor 的 SB 數量是否合理。如果某 floor SB 數量為 0，記錄 WARNING 但繼續。
...
python -m golden_scripts.tools.sb_validate \
    --sb-elements "{SB_SLIDES_INFO_DIR}/{fl}/calibrated/calibrated.json" \
    --config "{MODEL_CONFIG}" \
    --grid-data "{GRID_DATA}" \
    --output "{SB_SLIDES_INFO_DIR}/{fl}/calibrated/calibrated.json" \
...
python -m golden_scripts.tools.plot_elements \
    --elements "{SB_SLIDES_INFO_DIR}/{fl}/calibrated/calibrated.json" \
    --grid-data "{GRID_DATA}" \
    --output "{SB_SLIDES_INFO_DIR}/{fl}/calibrated/calibrated.png"
```
...
- **不修改 Phase 1 檔案**（SLIDES INFO/ 目錄只讀）
