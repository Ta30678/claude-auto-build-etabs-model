...
python -m golden_scripts.tools.sb_validate \
    ...
    --grid-data "{GRID_DATA_PATH}" \
    --output "{CASE_FOLDER}/sb_elements_validated.json" \
    --report "{CASE_FOLDER}/sb_validate_report.json"

# Step 2: 生成 sb_patch.json
python -m golden_scripts.tools.sb_patch_build \
    --sb-elements "{CASE_FOLDER}/sb_elements_validated.json" \
    --config "{CASE_FOLDER}/model_config.json" \
    --output "{CASE_FOLDER}/sb_patch.json"

# Step 3: Merge base + SB patch
...
python -m golden_scripts.tools.slab_generator \
    --config "{CASE_FOLDER}/merged_config.json" \
    --slab-thickness {SLAB_THICKNESS} --raft-thickness {RAFT_THICKNESS} \
    --output "{CASE_FOLDER}/final_config.json"
```

**RUN_SB_PIPELINE 訊息包含**：`CASE_FOLDER`, `GRID_DATA_PATH`, `SLAB_THICKNESS`, `RAFT_THICKNESS` 參數。
