...
6. **Phase 2 物件完全忽略。** 圖面/圖例中的小梁(SB/FSB/小梁/次梁)和樓板(S/FS/版/板)
   屬於 Phase 2 (`/bts-sb`) 範圍。即使在圖面上看到這些構件：
   - **不要在 SendMessage 中提及它們**
   - **不要向用戶詢問它們**
   - **不要在 grid_info.json 中記錄它們**
   - 圖例中的板/版色彩條目由腳本自動處理，不需人工介入
