...
## 鐵則（ABSOLUTE RULES — 所有 Agent 必須遵守，違反即失敗）

1. **小梁位置絕對禁止用 1/2、1/3 grid 間距猜測或假設等間距配置！** 必須從結構配置圖逐根量測像素位置，用 Grid Line 像素座標等比例插值計算精確座標（cm 級）。小梁位置由住宅單元隔間決定，每根都不同。如果 SB-READER 的輸出全部落在等分位置，Team Lead 必須退回要求重新量測。圖面解析度不足時，必須回報「無法量測」並請求更清晰圖面，絕不可用假設值。

2. **結構配置必須從結構配置圖讀取，禁止從舊模型複製或按比例縮放。** 新模型的所有構件位置由圖面決定。

3. **建模前必須交叉比對結構配置圖和建築平面圖**，確認實際建物範圍（可能不是完整矩形）。不可假設所有 Grid 交叉點都有柱。

**Team Lead 的審查責任**：在將 READER/SB-READER 的輸出交給 MODELER 之前，必須檢查小梁座標是否為機械性的等分配置。如果是，退回重做。
...
8. **基礎樓層規則**：see CLAUDE.md 'Foundation Floor Rules' section
9. **逐區段確認小梁**：MODELER-B 必須對每個樓層區段分別與 SB-READER 確認小梁配置，禁止只讀最低樓層套用全部
10. **不使用 SDL 載重**：SDL 絕對不建立。所有附加靜載重使用 DL pattern
